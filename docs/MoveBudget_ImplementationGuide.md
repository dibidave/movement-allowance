...
In **V13**, do **not** populate `CONFIG.Actor.trackableAttributes` with synthetic per-type entries when the game system relies on **DataModel / schema** for trackables: if that object exists for a type, core can omit schema-derived attributes from the Token Config **Resources** list (only CONFIG paths appear). This module instead **patches** `TokenDocument.getTrackedAttributes` to append **segment-array** paths (`["flags","movement-allowance","allowanceCurrent"]` and max) to `descriptor.bar` after core runs — `getTrackedAttributeChoices` expects each `bar` row to be `string[]` and calls `.join(".")` on it (plain dot strings throw `v.join is not a function`). It also **patches** `TokenDocument.prototype.getBarAttribute` so the chosen **current** path (string or segments) resolves `value` / `max` from module flags.
