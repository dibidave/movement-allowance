...
MoveBudget is a system-agnostic Foundry VTT module that enforces per-token movement limits during play. It tracks how far a token has moved per turn, blocks further movement once that limit is exhausted, and gives the GM simple accessible controls to manage and reset those limits at any time.
...
- Each token has a defined movement allowance per turn (`moveBudgetMax`).
- A live remaining value (`moveBudgetCurrent`) depletes as the token moves.
- When `moveBudgetCurrent` reaches zero, further movement is blocked.
...
| Term | Meaning |
|---|---|
| `moveBudgetMax` | The total movement allowance per turn — the ceiling the token resets to. |
| `moveBudgetCurrent` | The live remaining movement for this turn. This is the value that depletes. |
| `move-budget` | The module namespace. All flags and settings keys are prefixed with this. |
...
> **⚠️ Do not use the bare word `speed` anywhere in the codebase.** Almost every RPG system already defines an attribute named `speed` on their actor data model. Using it as a bare key risks collisions. All values must be stored under the `move-budget` namespace.
...
Both `moveBudgetMax` and `moveBudgetCurrent` are stored as flags on the **Actor document**, not on the Token document. This is a critical architectural decision.
...
Register MoveBudget as a token attribute by surfacing it as a `value`/`max` pair in the Actor's registered attributes. This makes it available natively in Foundry's token resource bar system with no custom bar rendering code required.
...
- The bar depletes and fills visually as `moveBudgetCurrent` changes.

> **Bar Slot Limitation:** Foundry natively supports two token bar slots (Bar 1 below the token, Bar 2 above). MoveBudget will occupy one of these slots. Users running the Bar Brawl module can add it as an additional bar without displacing existing ones. Document this in the module readme.

### 3.6 Grid Units

MoveBudget values are entered and displayed in the scene's native distance units, inherited directly from `canvas.grid.measurePath()` output. No separate unit conversion or label system is implemented in V1. The GM sets `moveBudgetMax` in the same units the scene uses, keeping the system consistent with native Foundry behavior.

---

## 4. Data Architecture

The following flags are stored on each Actor document under the `move-budget` namespace:

| Flag Key | Type | Description |
|---|---|---|
| `flags.move-budget.moveBudgetMax` | number | Total movement allowance per turn (the ceiling). |
| `flags.move-budget.moveBudgetCurrent` | number | Remaining movement this turn (live depleting value). |
| `flags.move-budget.exempt` | boolean | Skip all enforcement for this token. |

`moveBudgetMax` is set by the GM and represents the value the token resets to at the start of each turn. It is configured before play begins and changed infrequently. `moveBudgetCurrent` is the live value that changes constantly during play and is the value displayed on the token bar.
...
| `defaultMoveBudget` | Number | `30` | World | Default `moveBudgetMax` for any token without a per-token override. |
...
Inject a new **Movement** tab into the Token Config sheet (the window opened by right-clicking a token and selecting Open Configuration). Use the `renderTokenConfig` hook for this injection.

This tab must only render when `game.user.isGM` is `true`. Players never see it.

### 6.1 Tab Contents

#### MoveBudget Override (numeric input)

A numeric input field. When populated, this value overrides the module's Default MoveBudget setting for this specific token. It sets `moveBudgetMax` on the token's Actor. Leave blank to inherit the module default.
...
When checked, sets `flags.move-budget.exempt` to `true` on the Actor. This token will never have movement blocked regardless of any other setting. Its budget is not tracked or deducted.

### 6.2 Implementation Notes

- Use the `renderTokenConfig` hook to inject the tab HTML and wire up form submission.
- All form field IDs in this tab must use the module namespace as a prefix (e.g. `move-budget-max`, `move-budget-exempt`) to avoid conflicts with other modules injecting into the same sheet.
...
| 3 | Token Actor has `exempt` flag = `true` | No enforcement for this token. |
...
| 6 | Move distance > `moveBudgetCurrent` | Return `false`. Move blocked. Token snaps back silently. |
| 7 | All checks passed, move is within budget | Allow move. Deduct distance from `moveBudgetCurrent`. |
...
When a move passes all checks and is within budget (step 7), deduct the move distance from `moveBudgetCurrent` using an Actor flag update.
...
Foundry's native undo system (Ctrl+Z) restores a token's position but has no awareness of module flags. If a player undoes a valid move, the token returns to its prior position but `moveBudgetCurrent` remains at the post-move (lower) value — the budget is not restored.

This is a known limitation and is accepted in V1. The GM can correct this manually by editing the token's MoveBudget override in the Movement tab, or by using the Reset button on the control panel. Document this limitation in the module readme.
...
A single button labeled **Reset All Movement**. When clicked, it iterates over the appropriate token set (determined by the `resetScope` setting) and sets each tracked, non-exempt token's `moveBudgetCurrent` equal to its `moveBudgetMax`. This represents the start of a new turn or round.
...
Because `moveBudgetMax` and `moveBudgetCurrent` are stored on the Actor document, the data persists naturally when a linked token appears in a new scene. No additional hook handling is required. The token carries its current remaining budget into the new scene exactly as intended.
...
- **Undo does not restore MoveBudget.** Foundry's Ctrl+Z restores token position but not module flags. See Section 7.3.
- **Two native token bar slots.** MoveBudget occupies one of Foundry's two native bar slots. The Bar Brawl module resolves this for users who need additional bars. Note this in the module readme.
...
- **No unit label system.** MoveBudget values use the scene's native units. No separate labeling layer is implemented in V1.
